...
def topic_exists(client, topic_name):
    ...
    created_topics = client.list_topics().topics
    return topic_name in created_topics
...
async def produce(topic_name):
    ...
    p = Producer({"bootstrap.servers": BROKER_URL, "linger.ms": 2000})
...
async def consume(topic_name):
    ...
    while True:
        message = c.poll(
            0.05
        )  # QUERY : changing poll changes messages received. While I am polling, am i stopping messages from being received from the producer?
        if message is None:
